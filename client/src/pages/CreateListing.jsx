...
const CreateListing = () => {
  ...
  const handleDragPhoto = (result) => {
    if (!result.destinition) return;

    const items = Array.form(photos);
    const [reorderedItem] = items.splice(result.source.index, 1);
    items.splice(result.destinition.index, 0, reorderedItem);
  ...
  const handlePost = async (e) => {
    ...
    try {
      /* !--------- Create FormData object to handle input file uploads !---------- */
      ...
      const response = await fetch(`/api/properties/createListing`, {
        method: "POST",
        body: listingForm,
      });
